...
const buildMessage = (lang) => {
  ...
  message += `${lang === 'en' ? '👨‍🍳 Name' : '👨‍🍳 Повар'}: ${name}\n\n`;
...
button.addEventListener('click', () => {
  ...
  // Отправка email через Web3Forms с фиксированным отправителем SHBB и динамической темой
    ...
    try {
      const res = await fetch("https://api.web3forms.com/submit", {
        ...
        body: JSON.stringify({
          ...
          subject: subject,         // динамическая тема
          from_name: "SHBB",        // всегда SHBB
  ...
  const sendAllParts = async (text, subject) => {
    let start = 0;
    while (start < text.length) {
      const chunk = text.slice(start, start + 4000);
      await sendMessage(chunk);
      await sendEmail(chunk, subject);
      start += 4000;
    }
  };
  ...
  (async () => {
    try {
      await sendAllParts(buildMessage('ru'), "кондитерка_отдано");
      await sendAllParts(buildMessage('en'), "кондитерка_given");
